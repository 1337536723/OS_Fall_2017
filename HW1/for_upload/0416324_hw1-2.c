...
UNT* assign_shared_mem(int);
void detach_shm(UNT*);
void release_all_shm(UNT*);
int main()
{
    ...
    UNT* shm_addr_a = assign_shared_mem(dim);
    for(int row=0;row<dim;row++)
        for(int col=0;col<dim;col++)
            shm_addr_a[row*dim+col]=row*dim+col;
    /*----------------------------------------------------------------------------------*/
    //printf("Enter matrix B: ");
    UNT* shm_addr_b = assign_shared_mem(dim);
    for(int row=0;row<dim;row++)
        for(int col=0;col<dim;col++)
            shm_addr_b[row*dim+col]=row*dim+col;

    UNT* shm_addr_c = assign_shared_mem(dim);
...
UNT* assign_shared_mem(int dim) //return the address of the shared memory
{
...
void release_all_shm(UNT* shm_addr)
{
    //detach_shm(shm_addr);
